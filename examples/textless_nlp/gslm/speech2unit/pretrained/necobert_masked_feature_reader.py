...
class NecoBERTMaskedFeatureReader:
    """
    ...
    def __init__(self,checkpoint_path, layer, max_chunk=240000, use_cuda=True) -> None:
        super().__init__()
        self.model = transformers.AutoModel.from_pretrained('Wataru/necobert-base-masked', trust_remote_code=True).eval()
    ...
    def get_feats(self, file_path, ref_len=None, channel_id=None):
        ...
        with torch.no_grad():
            ...
            for start in range(0, x.size(1), self.max_chunk):
                ...
                feat_chunk= self.model({"x": x_chunk.view(1,1,-1)},sample_rate=24000).hidden_states[self.layer]
